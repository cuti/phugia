...
class Default_Model_Product extends Zend_Db_Table_Abstract {
  ...
  public function getProductIdByProductCode($productCode) {
    $select  = $this->select()->from($this)->where('product_code = ?', trim($productCode));
  ...
  public function getProductByProductId($productId) {
    $select  = $this->select(true)->where('product_id = ?', $productId);
    $product = $this->fetchRow($select);
    return $product;
  }
